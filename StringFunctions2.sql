...
-- SELECT SUBSTRING(first_name, 2, 3) as sub_first_name,
-- SUBSTRING(last_name, 3, 2) as sub_last_name
-- from DIRECTORS;



----- REPLACE function can change the actual data in the table iff we use it to update a table 
----- REPLACE('source_string', 'old_string', 'new_string');

-- SELECT REPLACE('A cat jumped over another cat', 'cat', 'dog');

-- SELECT first_name, last_name, REPLACE(gender, 'M', 'Male') AS Gen from actors;


-- SELECT split_part(first_name, 1, 2) from actors;


SELECT movie_name, split_part(movie_name, ' ', 1) AS first_word,
split_part(movie_name, ' ', 2) AS second_word from movies;
